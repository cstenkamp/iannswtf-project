...
def make_dataset():
    ...
    forreverse["UNK"] = len(forreverse)+1
    reverse_dictionary = dict(zip(forreverse.values(), forreverse.keys()))
    for k,v in allwords.items():
        if v < 0:
            allwords[k] = len(forreverse)       
    reverse_dictionary[len(reverse_dictionary)+1] = "UNK"    
...
my_file = Path("./ratings_ohne_wordvecs.pkl")
...
#print(moviedat.lookup)


#TODO: Replace rare words with UNK token?
...
def generate_batch(batch_size, num_skips, skip_window, dataset):
  ...
  for _ in range(span):
    global permutation
    buffer.append(dataset[permutation[dindex[0]]][dindex[1]])
    dindex[1] = dindex[1] + 1
...
with graph.as_default():
  ...
  with tf.device('/cpu:0'):
    ...
    nce_weights = tf.Variable(
        tf.truncated_normal([moviedat.ohnum, embedding_size],
                            stddev=1.0 / math.sqrt(embedding_size)))
    nce_biases = tf.Variable(tf.zeros([moviedat.ohnum]))
...
with tf.Session(graph=graph) as session:
  ...
  for step in xrange(num_steps):
    batch_inputs, batch_labels = generate_batch(
        batch_size, num_skips, skip_window, dataset=moviedat.reviews)
    ...
    if step % 10000 == 0:
      ...
      for i in xrange(valid_size):
        valid_word = moviedat.uplook[valid_examples[i]]
        top_k = 8  # number of nearest neighbors
        nearest = (-sim[i, :]).argsort()[1:top_k + 1]
        log_str = "Nearest to %s:" % valid_word
        for k in xrange(top_k):
          close_word = moviedat.uplook[nearest[k]]
          log_str = "%s %s," % (log_str, close_word)
        print(log_str)
  final_embeddings = normalized_embeddings.eval()

  moviedat.add_wordvecs(final_embeddings)
...
try:
  ...
  labels = [moviedat.uplook[i] for i in xrange(plot_only)]
